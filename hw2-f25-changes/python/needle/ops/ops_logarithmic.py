...
class LogSoftmax(TensorOp):
    def compute(self, Z: NDArray) -> NDArray:
        ### BEGIN YOUR SOLUTION
        max_vals = array_api.max(Z, axis=1, keepdims=True)
        shifted = Z - max_vals
        sum_exp = array_api.sum(array_api.exp(shifted), axis=1, keepdims=True)
        log_sum_exp = max_vals + array_api.log(sum_exp)
...
class LogSumExp(TensorOp):
    ...
    def compute(self, Z: NDArray) -> NDArray:
        ...
        if self.axes is None:
            max_val = array_api.max(Z)
            return array_api.log(array_api.sum(array_api.exp(Z - max_val))) + max_val
        else:
            max_vals = array_api.max(Z, axis=self.axes, keepdims=True)
            exp_shifted = array_api.exp(Z - max_vals)
            sum_exp = array_api.sum(exp_shifted, axis=self.axes, keepdims=True)
            result = max_vals + array_api.log(sum_exp)
            ...
            result = array_api.reshape(result, result_shape)
